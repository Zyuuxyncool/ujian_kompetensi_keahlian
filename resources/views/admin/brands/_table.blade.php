<div class="table-responsive">
    <table class="table align-middle table-row-dashed fs-6 table-sm">
        ...
        @if($brands instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($brands->currentPage()-1) * $brands->perPage()) + 1)
        @endif
        @foreach($brands as $brand)
            <tr>
                ...
                <td class="py-0 align-middle">
                    <img src="{{ Storage::url($brand->photo) }}" alt="" class="h-30px w-30px object-fit-cover" />
                </td>
                <td class="text-end text-nowrap">
                    ...
                    <div class="menu menu-sub menu-sub-dropdown dropdown-menu menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-auto py-4" data-kt-menu="true">
                        <div class="menu-item px-3"><a onclick="info({{ $brand->id }})" href="javascript:void(0)" class="menu-link px-3">Edit</a></div>
                            <div class="menu-item px-3"><a onclick="confirm_delete({{ $brand->id }})" href="javascript:void(0)" class="menu-link px-3">Delete</a></div>
                    </div>
                </td>
            </tr>
        @endforeach
