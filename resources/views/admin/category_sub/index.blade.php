    ...
    <div class="content flex-column-fluid" id="kt_content">
        <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
            <div class="page-title d-flex flex-column py-1">
                <h1 class="d-flex align-items-center my-1"><span class="text-dark fw-bold fs-1">Category Sub</span></h1>
                @include('admin.layouts._breadcrumb')
            </div>
            <div class="d-flex align-items-center py-1 gap-6">
                <button type="button" onclick="info()" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-40px">Category Sub Baru</button>
            </div>
        </div>
                    ...
                    <form id="form_search" class="w-100">
                        ...
                        <div class="card-title d-flex flex-row align-items-center gap-4">
                            <div class="d-flex align-items-center position-relative gap-6 w-100 w-lg-250px">
                                ...
                                <x-input name="name" prefix="search_" caption="Search Category Sub" class="w-lg-250px ps-12" />
                            </div>
                            <x-select name="category_id" :options="$list_category" class="w-lg-250px form-select" onchange="search_data()" />
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('admin.category_sub.index') }}';
